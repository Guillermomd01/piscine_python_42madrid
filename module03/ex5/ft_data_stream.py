def events_processor(event_list: list):
...
print(f"Processing {len(event_list)} game events...\n")
for event in events_processor(event_list):
    events += 1
    print(
...
i = 0
sequence_fibonacci = ""
for n in finbonacci_generator():
    i += 1
    if i <= 10:
        if sequence_fibonacci == "":
            sequence_fibonacci = f"{n}"
        else:
            sequence_fibonacci += f", {n}"
    else:
        break
...
print(f"Fibonacci sequence (first 10): {sequence_fibonacci}")
print(f"Prime numbers (first 5): {sequence_prime}")
