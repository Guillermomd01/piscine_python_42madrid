    ...
    def garden_report(self):
        ...
        for plant in self.plants:
            if plant.prize_points > 0:
                ...
                regular += 1
        print(
            f"Plants added: {GardenManager.GardenStats.total_plant(self)}, "
            f"Total growth: {GardenManager.GardenStats.total_grow(self)}cm")
        print(
            f"Plant types: {regular} regular, {flowering} flowering, {prize}"
            f"prize flowers\n")
        print(
            f"Height validation test: "
            f"{GardenManager.height_validation_test(self)}"
            )
