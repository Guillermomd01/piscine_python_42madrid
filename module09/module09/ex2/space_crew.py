...
class SpaceMission(BaseModel):
    ...
    def mission_validation_rules(self):
        ...
        leader = any(m.rank in [Rank.captain, Rank.commander] for m in self.crew)
        if not leader:
            raise ValueError("Mission must have at least one Commander or Captain")
        len_crew = len(self.crew)
        most_experience = [member.years_experience for member in self.crew if member.years_experience >= 5]
